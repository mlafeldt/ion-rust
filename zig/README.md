...
- `zig/src/ion/writer11.zig` (experimental / partial)
  ...
  - Provides low-level helpers for emitting conformance-driven Ion 1.1 binary e-expressions:
    ...
    - Qualified system macro invocations (`0xEF`) for a small subset of system macros (currently including `values`, `default`, `repeat`, `delta`, `sum`, `annotate`, `make_decimal`, `make_timestamp`, `flatten`, `set_symbols`, `add_symbols`, `set_macros`, `add_macros`, and `use`).
...
- As of 2025-12-26, `cd zig && zig build test --summary all` runs 90 Zig tests; all pass (0 skips).
